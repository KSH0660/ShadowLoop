...
- **Phase 3~4 완료**: Friends 15 + The Office 2 + Big Bang 1 추가 + 글로싱 → **누계 48개**(목표 40~50 범위). 마지막 8개 글로싱은 서브에이전트 2개 병렬 작업 후 메인에서 타입 정규화·머지. curator 스킬의 `web_embedded` 우회로 클라우드에서도 자막 다운로드 성공.
- **Phase 8 착수**: 견본 1개 영상(ykN0VZiEDr8) 해설 4건을 풍부 형식으로 전환. 나머지 기존 해설은 배치 반복으로 후속 진행(파이프라인 `npm run build:glossary` 동작 확인).
- **남은 것**: Phase 8 기존 영상 해설 풍부화 배치(선택).
...
- [x] Friends 7 + Big Bang Theory 1(TBS) 추가로 **48개** 도달(목표 40~50 범위 안). 글로싱은 서브에이전트 2개 병렬 작업 후 메인에서 머지.
- [x] 글로싱 + 빌드 + check.
- 완료 기준: 누계 **48개**. 카테고리 — 프렌즈 15 / 모던 패밀리 14 / TED 5 / 키앤필 4 / 멀레이니 3 / 타블로 2 / 하산 민하지 2 / 오피스 2 / 빅뱅이론 1.
